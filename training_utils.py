...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_args, tokenizer: transformers.PreTrainedTokenizer):
        ...
        if data_path == 'meta-math/MetaMathQA':
            ...
            list_data_dict = load_dataset(data_args.data_path, split=data_args.dataset_split if data_args.dataset_split else "train").to_list()
        elif data_path == 'fxmeng/CodeFeedback-Python105K':
            from datasets import load_dataset
            # list_data_dict = load_dataset('meta-math/MetaMathQA')['train'].to_list()
            list_data_dict = load_dataset(data_args.data_path, split=data_args.dataset_split if data_args.dataset_split else "train").to_list()
        else:
...
def tokenize(prompt, add_eos_token=True, cutoff_len=256, tokenizer=None):
...
def generate_and_tokenize_prompt(data_point, train_on_inputs=True, tokenizer=None):
    full_prompt = generate_prompt(data_point)
    tokenized_full_prompt = tokenize(full_prompt,tokenizer=tokenizer)
    if not train_on_inputs:
        user_prompt = generate_prompt({**data_point, "output": ""})
        tokenized_user_prompt = tokenize(user_prompt, add_eos_token=False,tokenizer=tokenizer)
...
def load_commonsense_datasets(data_path,tokenizer):
    from datasets import load_dataset
    val_set_size = 120
    generate_and_tokenize_prompt_by_tokenizer = partial(generate_and_tokenize_prompt,tokenizer=tokenizer)
    ...
    if val_set_size > 0:
        ...
        train_data = (
            train_val["train"].shuffle().map(generate_and_tokenize_prompt_by_tokenizer)
        )
        val_data = (
            train_val["test"].shuffle().map(generate_and_tokenize_prompt_by_tokenizer)
        )
    else:
        train_data = data["train"].shuffle().map(generate_and_tokenize_prompt_by_tokenizer)
        val_data = None
    return train_data, val_data
def print_trainable_parameters_and_names(model):
    import pdb
    for name, param in model.named_parameters():
        if param.requires_grad:
            print(f"Name: {name}, Shape: {param.shape}")

class LinearOutputNormTracker:
    ...
    def add_hooks(self):
        def save_output_norm(layer_name):
            def hook(module, input, output):
                norms = torch.linalg.norm(output.view(-1, output.shape[-1]), dim=0).detach()
                if layer_name not in self.output_norms.keys():
                    self.output_norms[layer_name] = torch.zeros_like(norms)
                dist.all_reduce(norms, op=dist.ReduceOp.SUM)
                world_size = dist.get_world_size()
                activation_mean = norms / world_size
                self.output_norms[layer_name] = self.output_norms[layer_name]*0.9 + activation_mean*0.1
            return hook

        for name, module in self.model.named_modules():
            if isinstance(module, nn.Linear):
                hook = module.register_forward_hook(save_output_norm(name))
    ...
    def get_output_norms(self):
        return self.output_norms

class CosineAnnealingThreshold:
    def __init__(self, T_max, eta_max=0.05, eta_min=0.0):
        self.T_max = T_max
    ...
    def get_threshold(self):
        ...
        cos_inner = (math.pi * self.current_step) / self.T_max
        return self.eta_min + (self.eta_max - self.eta_min) * (1 + math.cos(cos_inner)) / 2
