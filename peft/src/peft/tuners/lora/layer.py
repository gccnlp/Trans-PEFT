...
class LoraLayer(BaseTunerLayer):
    ...
    def __init__(self, base_layer: nn.Module, **kwargs) -> None:
        ...
        self.plora_m: dict[str, float] = {}  # for plora
        self.lora_gate = nn.ParameterDict({})
        self.lora_rank_mixer = nn.ParameterDict({})
        #self.ranknum = nn.ParameterDict({}) # for alora
        self.use_beamlora: dict[str, bool] = {}
        self.use_pissa: dict[str, bool] = {}
        self.use_erank: dict[str, bool] = {}
        self.record_activations = False
        self.record_first_order_importance = False
    ...
    def update_layer(
        ...
        use_mora: bool = False, use_beamlora: bool = False, use_erank: bool = False, mora_type: int = 1, plora_m: float = 0.0, use_pissa: bool = False,
        record_activations: bool = False, record_first_order_importance: bool = False,
    ):
        ...
        self.plora_m[adapter_name] = plora_m
        self.record_activations = record_activations
        self.record_first_order_importance = record_first_order_importance
        #print(record_activations)
        ...
        if use_mora:
            ...
            if use_beamlora:
                self.lora_gate[adapter_name] = nn.Parameter(torch.randn(1, r), requires_grad=True)
                #self.lora_rank_mixer[adapter_name] = nn.Parameter(torch.zeros(r, r), requires_grad=True)
                self.adapter_layer_names = self.adapter_layer_names[:] + ("lora_gate",)
                #self.adapter_layer_names = self.adapter_layer_names[:] + ("lora_rank_mixer",)
                self.use_beamlora[adapter_name] = True
                # The current rank
                # self.ranknum[adapter_name] = nn.Parameter(torch.randn(1), requires_grad=False)
                # self.ranknum[adapter_name].data.fill_(float(r))
                # self.ranknum[adapter_name].requires_grad = False
                # self.other_param_names = self.other_param_names[:] + ("ranknum",)
                # self.scaling[adapter_name] = lora_alpha if lora_alpha > 0 else float(r)
            else:
                self.use_beamlora[adapter_name] = False
            self.use_pissa[adapter_name] = use_pissa
            self.use_erank[adapter_name] = use_erank
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights, mora_type=None):
        def moe_style_init(weight):
            print("use moe init")
            
            #print(weight.data.shape)
            group_size = 16
            init_row_vector = torch.empty(group_size, weight.data.shape[1], device=weight.device)

            
            nn.init.kaiming_uniform_(init_row_vector, a=math.sqrt(5))
            
            with torch.no_grad():
                
                new_weight = init_row_vector.repeat(weight.data.shape[0] // group_size, 1)
                weight.copy_(new_weight)
        def svd_init(top_r=True):
            print("Use Pissa Init!Top R: ",str(top_r))
            ...
            self.get_base_layer().weight.data = weight.to(dtype)
        ...
        if adapter_name in self.lora_A.keys():
            if self.use_beamlora[adapter_name]:
                #nn.init.normal_(self.lora_gate[adapter_name], mean=1.0, std=0.002)
                #nn.init.ones_(self.lora_gate[adapter_name])
                #nn.init.kaiming_uniform_(self.lora_gate[adapter_name], a=math.sqrt(5))
                #nn.init.kaiming_uniform_(self.lora_rank_mixer[adapter_name], a=math.sqrt(5))
                #moe_style_init(self.lora_A[adapter_name].weight)
                ...
                nn.init.zeros_(self.lora_gate[adapter_name])
            elif self.use_pissa[adapter_name]:
                svd_init(top_r=True)
            else:
...
class Linear(nn.Module, LoraLayer):
    ...
    def __init__(
        ...
        record_first_order_importance: bool = False,
        mora_type: int = 1,
        **kwargs,
    ) -> None:
        super().__init__()
        ...
        self.update_layer(
            ...
            use_mora=use_mora,
            plora_m=plora_m,
            use_beamlora=use_beamlora,
            use_erank=use_erank,
            use_pissa=use_pissa,
            mora_type=mora_type,
            record_activations=record_activations,
            record_first_order_importance=record_first_order_importance,
        )
        self.is_target_conv_1d_layer = is_target_conv_1d_layer
        self.recorded_activations = None
        self.temperature=0.05 # 0.05 for lr>=1e-4, 0.03 for lr<1e-4

    def merge(self, safe_merge: bool = False, adapter_names: Optional[List[str]] = None, use_plora=False) -> None:
        """
        ...
        for active_adapter in adapter_names:
            if active_adapter in self.lora_A.keys():
                ...
                if safe_merge:
                    ...
                    delta_weight = self.get_delta_weight(active_adapter) * (1-self.plora_m[active_adapter])
                ...
                ## Code for PLoRA
                if use_plora:
                    dtype = self.lora_B[active_adapter].weight.dtype
                    device =  self.lora_B[active_adapter].weight.device
                    self.lora_A[active_adapter].weight.data = (self.lora_A[active_adapter].weight.data * math.sqrt(self.plora_m[active_adapter])).to(dtype)
                    self.lora_B[active_adapter].weight.data = (self.lora_B[active_adapter].weight.data * math.sqrt(self.plora_m[active_adapter])).to(dtype)
                else:
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        if self.use_mora[adapter]:
            ...
            output_tensor = w
        elif self.use_beamlora[adapter]:
            #output_tensor = transpose(weight_B @ (weight_A.mul(self.lora_gate[adapter].T)), self.fan_in_fan_out) * self.scaling[adapter]
            
            output_tensor = transpose(weight_B @ (weight_A.mul(F.softmax(self.lora_gate[adapter]/self.temperature).T)*self.r[adapter]), self.fan_in_fan_out) * self.scaling[adapter]
            
            #output_tensor = transpose(weight_B @ (self.lora_rank_mixer[adapter] @ weight_A), self.fan_in_fan_out) * self.scaling[adapter]
        else:
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        #     result = F.linear(x, ow+w, self.base_layer.bias)
            ...
            for active_adapter in self.active_adapters:
                ...
                if self.use_mora[active_adapter]:
                ...
                elif self.use_beamlora[active_adapter]:

                    # ranknum = self.ranknum[active_adapter] + 1e-5
                    x = dropout(x)
                    
                    #result = result + lora_B(lora_A(x)* self.lora_gate[active_adapter]) * scaling
                    result = result + lora_B(lora_A(x)* F.softmax(self.lora_gate[active_adapter]/self.temperature)*self.r[active_adapter]) * scaling
                    
                    #result = result + lora_B(lora_A(x)@self.lora_rank_mixer[active_adapter].T) * scaling

                elif self.record_activations:
                    x = dropout(x)
                    output_A = lora_A(x)  # (batch_size, r)
                    norms = torch.linalg.norm(output_A.view(-1,self.r[active_adapter]), dim=0).detach()  
                    ...
                    activation_mean = norms / world_size
                    self.recorded_activations = self.recorded_activations*0.9 + activation_mean*0.1
                    if self.use_erank[active_adapter]:
                        output_A = output_A * (F.softmax(self.recorded_activations.unsqueeze(0)/self.temperature)*self.r[active_adapter])
                    result = result + lora_B(output_A) * scaling
