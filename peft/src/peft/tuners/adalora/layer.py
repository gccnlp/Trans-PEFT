    ...
    def _set_budget_scheduler_increlora(self, model, init_r):
        ...
        for name,module in model.named_modules():
            if hasattr(module,"lora_E"):
                ...
                lora_A.data[:init_r,:] = torch.normal(mean=0, std=0.02, size=(init_r, lora_A.data.size(1)))
                lora_A.data[init_r:,:] = torch.zeros_like(lora_A.data[init_r:,:])
                self.name_set.add(name)
                lora_B.data[:,:init_r] = torch.normal(mean=0, std=0.02, size=(lora_B.data.size(0), init_r))
                lora_B.data[:,init_r:] = torch.zeros_like(lora_B.data[:,init_r:])
            ...
            if not self.use_increlora:
                ...
                for name,module in model.named_modules():
                    if isinstance(module, SVDLinear):
                        if triplet_ipt[name+f".lora_E.{self.adapter_name}"] >= increase_threshold:
                            add_r = self.incre_rank_num
                            old_r = int(module.ranknum[self.adapter_name].data.item())
                            lora_A = module.lora_A[self.adapter_name]
                            lora_B = module.lora_B[self.adapter_name]
                            lora_A.data[old_r:old_r+add_r,:] = torch.normal(mean=0, std=0.02, size=(add_r, lora_A.data.size(1)))
                            lora_B.data[:,old_r:old_r+add_r] = torch.normal(mean=0, std=0.02, size=(lora_B.data.size(0), add_r))
                            module.ranknum[self.adapter_name].data += add_r
                            total_r = old_r + add_r
                            module.scaling[self.adapter_name] = 2*float(total_r)
                            pattern = [True] * total_r + [False] * (lora_A.size(0) - total_r)
