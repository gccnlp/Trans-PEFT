...
class AdaLoraModel(LoraModel):
    ...
    **Attributes**:
    ...
    def __init__(self, model, config, adapter_name):
        ...
        self.use_increlora = self.peft_config[adapter_name].use_increlora
    ...
    def _create_and_replace(
        ...
        kwargs = {
            "r": lora_config.init_r if not lora_config.use_increlora else lora_config.init_r*8,
            "lora_alpha": lora_config.lora_alpha,
            "lora_dropout": lora_config.lora_dropout,
            "fan_in_fan_out": lora_config.fan_in_fan_out,
            "init_lora_weights": lora_config.init_lora_weights,
            "use_increlora": lora_config.use_increlora,
        ...
        if not isinstance(target, AdaLoraLayer):
            ...
            target.update_layer(
                adapter_name,
                lora_config.init_r if not lora_config.use_increlora else lora_config.init_r*8,
                lora_config.lora_alpha,
                lora_config.lora_dropout,
                lora_config.init_lora_weights,
                lora_config.use_increlora,
            )
    ...
    def forward(self, *args, **kwargs):
        ...
        if (getattr(outputs, "loss", None) is not None) and isinstance(outputs.loss, torch.Tensor):
            ...
            use_increlora = self.peft_config[self.trainable_adapter_name].use_increlora
            ...
            if use_increlora:
                for name,module in self.model.named_modules():
                    if hasattr(module,"lora_E"):
                        r = int(module.ranknum[self.trainable_adapter_name].data.item())
                        p = module.lora_A[self.trainable_adapter_name].data[:r,:]
                        para_cov = p @ p.T
                        I = torch.eye(*para_cov.size(), out=torch.empty_like(para_cov))  # noqa: E741
                        I.requires_grad = False
                        num_param += 1
                        regu_loss += torch.norm(para_cov - I, p="fro")
                        p = module.lora_B[self.trainable_adapter_name].data[:,:r]
                        para_cov = p.T @ p
                        I = torch.eye(*para_cov.size(), out=torch.empty_like(para_cov))  # noqa: E741
                        I.requires_grad = False
                        num_param += 1
